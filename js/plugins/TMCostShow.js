...
(function() {
  ...
  Window_SkillList.prototype.drawSkillCost = function(skill, x, y, width) {
    ...
    if (cost > 0) {
      x = this.drawSkillTpCost(cost, x + 2, y);//edited
    ...
    if (cost > 0) {
      x = this.drawSkillMpCost(cost, x + 2, y, count);//edited
      if (count++ === maxCostNum - 1) {
        return;
      }
    }
    if (Imported.TMSkillCostEx) {
      ...
      if (cost > 0) {
        x = this.drawSkillHpCost(cost, x + 2, y, count);//edited
        if (count++ === maxCostNum - 1) {
          return;
        }
      }
