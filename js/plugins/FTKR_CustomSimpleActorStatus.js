...
(function() {
    ...
    Window_Base.prototype.drawCssActorStatus = function(index, actor, x, y, width, height, lss) {
        /*  added 5 lines below 2021.01.07 NEKOMA */
        var bitmaps = actor._cssbgi.map(bgi => ImageManager.loadPicture(bgi.name)).filter(bitmap => bitmap.width == 0);
        if (bitmaps.length > 0) {
            bitmaps[0].addLoadListener(this.drawCssActorStatus.bind(this, index, actor, x, y, width, height, lss));
            return;
        }
